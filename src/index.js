...
client.on("message", async (msg) => {
  ...
  const isGetImageColorCommand = () => {
    ...
    return colorCommands.some((command) => msg.content.includes(command));
  };
  if (isGetImageColorCommand()) {
    ...
    if (RefMessage == undefined) {
      return msg.lineReply(
        "Please, use this command by replying to a message with an image!"
      );
    }
    ...
    if (isImageUrl(ImgUrl)) {
      ...
      if (!colorsObj) {
        return msg.lineReply(
          "Sorry but Parrots only accepts jpeg or png Images!"
        );
      }
      ...
      await msg.lineReply("GoGo Robo-Parrot found some colors...", {
        files: ["../color-parrot-discord-bot/" + file],
      });
      ...
      return msg.lineReply(
        "Humm... Something went wrong,I think this is not an image. "
      );
    }
  }
  if (command == "more") {
    ...
    if (InitialMessage == undefined) {
      return msg.lineReply(
        `Please, use this command by replying to my answer (First Use ${prefix}GetColor replying to the image).`
      );
    }
    ...
    if (isImageUrl(ImgUrl)) {
      ...
      if (Colors.length <= 1) {
        return msg.lineReply("Hummm... This image no longer has colors!");
      }
      if (Colors.length <= 9) {
        msg.lineReply(
          `Hummm...This image doesn't have more than 9 colors... So I'll send all it has.`
        );
      }
      ...
      if (!paletteImageObj) {
        return msg.lineReply(
          "Humm...I think there are no more colors in this image, sorry."
        );
      }
      const file = await base64_decode(JSON.stringify(paletteImageObj));
      await msg.lineReply("GoGo Robo-Parrot found some more awesome colors!", {
        files: ["../color-parrot-discord-bot/" + file],
      });
      //After send the message with the Image, unlink file...
      fs.unlink(file, (err) => {
  ...
  const nameCommands = ["name", "what is the name of", "what's the name of"];
  if (nameCommands.some((command) => msg.content.includes(command))) {
  ...
  if (command == "help") {
    if (command == "help") {
      const HelpEmbed = new Discord.MessageEmbed()
        ...
        .setDescription(
          `My prefix is "**${prefix}**" or you can just mention me (Use the prefix or mention before the command).`
        )
        .addField(
          "Want the parrots in your server too?",
          "Our robo-parrots are happy to do help you ![Add me to your server!](https://discord.com/oauth2/authorize?client_id=873156023903457280&permissions=51200&scope=bot)",
          true
        )
        .setColor("#7a58c1")
        .setThumbnail(
          "https://pbs.twimg.com/profile_images/1390699453934342156/Zo1enErC.jpg"
        )
        .setImage(
          "https://pbs.twimg.com/profile_banners/1109778441379950592/1627417955/1080x360"
        )
        .addFields(
          {
            name: "Get the color Of an Image",
            value: "`getcolor, color, colors, what color is this...`",
          },
          {
            name: "Get the color name of an Hex",
            value: "`name, what is the name of, what's the name of.`",
          },
          {
            name: "Get more colors than you think",
            value: "`more...(Reply in the palette that i send...)`",
            inline: true,
          },
          { name: "Help Command", value: "`help... my command list.`" }
        )
        .addField(
          "Some Links:",
          "[GitHub](https://github.com/AlissonSteffens/color-parrot-discord-bot), [Twitter Version](https://twitter.com/color_parrot)"
        );
...
async function GetOriginalMessage(msg) {
  ...
  let InitialMessageWithTheImg = await GetReplyContent(
    MessageThatUserUseGetColor
  );
